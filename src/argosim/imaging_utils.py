...
def kaiser_bessel(x, W, beta):
    ...
    return jax.scipy.special.i0(beta * jnp.sqrt(arg + 1e-12)) / jax.scipy.special.i0(
        beta
    )
...
def check_uv_in_grid(
    uv_px, grid_shape, fov_size, on_out_of_bounds="warn"
):  # pragma: no cover
...
def check_uv_samples_range(
    uv_samples_indices, uv_samples, sky_uv_shape, fov_size
):  # pragma: no cover
...
def grid_uv_samples(
    uv_samples, sky_uv_shape, fov_size, mask_type="binary"
):  # pragma: no cover
    """Grid uv samples.
