...
class TestAntennaUtils:
    ...
    y_antenna_exp = np.array([[   100.        ,    0.        ,    0.        ],
                                [ 200.        ,    0.        ,    0.        ],
                                [ 300.        ,    0.        ,    0.        ],
                                [ 400.        ,    0.        ,    0.        ],
                                [ 500.        ,    0.        ,    0.        ],
    ...
    random_antenna_exp = np.array([[   196.4691856 , -213.86066505,    0.       ],
                                    [  51.31476908,  219.46896979,    0.        ],
                                    [ 480.76419838,  184.82973858,    0.        ]]
    )
    random_antenna_tolerance = 1e-7

    uni_antenna_span = {"east": 800.0, "north": 800.0, "up": 0.0}
    uni_antenna_test_element = 333
    uni_antenna_333 = np.array([ -64.51612903, -141.93548387,    0.        ])
    uni_antenna_tolerance = 1e-7

    ENU_antenna_in = np.array([[101., 200.5, -33.]])
    ...
    enu_txt_arr_exp = np.array([[ 15.,  10.,   0.],
                                [ 17.,  35.,   0.],
                                [  7.,  50.,   0.],
                                [ 17.,  75.,   0.],
                                [ 10., 183.,   0.]]
    ...
    latlon_txt_arr_exp = np.array([[-18159.17012419,  -5565.97453966,      0.],
                                   [ 18159.17012419,   5565.97453966,      0.]]
    )

    random_antenna_baselines_exp = np.array([   [ 145.15441652, -433.32963484,    0.        ],
                                                [-284.29501278, -398.69040363,    0.        ],
                                                [-145.15441652,  433.32963484,    0.        ],
                                                [-429.4494293 ,   34.63923121,    0.        ],
                                                [ 284.29501278,  398.69040363,    0.        ],
                                                [ 429.4494293 ,  -34.63923121,    0.        ]]
    )

    uv_track_default_exp = np.array([   [ 6.87539883e+02, -2.05251355e+03, -1.34622557e-13],
                                        [-1.34659464e+03, -1.88844101e+03, -1.34622557e-13],
                                        [-6.87539883e+02,  2.05251355e+03,  1.34622557e-13],
                                        [-2.03413453e+03,  1.64072534e+02,  0.00000000e+00],
                                        [ 1.34659464e+03,  1.88844101e+03,  0.00000000e+00],
                                        [ 2.03413453e+03, -1.64072534e+02,  1.68278197e-14]]
    )
    uv_track_params = (
        35.0 / 180 * np.pi, # lat
        35.0 / 180 * np.pi, # dec
    ...
    uv_track_exp = np.array([   [  509.40157076, -2024.3298598 ,    62.96406953],
    ...
    def test_uni_antenna_arr(self):
        ...
        npt.assert_allclose(
            ...
            err_msg="Uniform antenna output does not match at position {}.".format(self.uni_antenna_test_element)
        )

    def test_enu_to_xyz(self):
        x, y, z = au.ENU_to_XYZ(self.ENU_antenna_in)
        npt.assert_almost_equal(
            x, self.XYZ_antenna_exp["X"], decimal=self.ENU_to_XYZ_decimal_tol,
            err_msg="X coordinate conversion from ENU to XYZ does not match."
        )
        npt.assert_almost_equal(
            y, self.XYZ_antenna_exp["Y"], decimal=self.ENU_to_XYZ_decimal_tol,
            err_msg="Y coordinate conversion from ENU to XYZ does not match."
        )
        npt.assert_almost_equal(
            z, self.XYZ_antenna_exp["Z"], decimal=self.ENU_to_XYZ_decimal_tol,
    ...
    def test_xyz_to_uvw(self):
        ...
        npt.assert_almost_equal(
            u, self.uvw_exp["u"], decimal=self.XYZ_to_uvw_decimal_tol,
            err_msg="U coordinate conversion from XYZ to UVW does not match."
        )
        npt.assert_almost_equal(
            v, self.uvw_exp["v"], decimal=self.XYZ_to_uvw_decimal_tol,
            err_msg="V coordinate conversion from XYZ to UVW does not match."
        )
        npt.assert_almost_equal(
            w, self.uvw_exp["w"], decimal=self.XYZ_to_uvw_decimal_tol,
    ...
    def test_combine_antenna_arr(self):
        ...
        assert len(arr) == n_antenna_exp, "Combined antenna array length does not match expected."
    ...
    def test_uv_track_params(self):
        track, _ = au.uv_track_multiband(self.random_antenna_baselines_exp, *self.uv_track_params)
        npt.assert_allclose(
            track,
            self.uv_track_exp,
            err_msg="UV track computed from random antenna baselines does not match expected output."
    ...
    def test_uv_track_multiband(self):
        track, _ = au.uv_track_multiband(self.random_antenna_baselines_exp, *self.uv_track_params[:-1], multi_band=True)
